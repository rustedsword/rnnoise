#!/usr/bin/env bash
# Host-side launcher for building and publishing rnnoise to Launchpad.
...
# rnnoise tree *is* the source. To move to a newer upstream commit, rebase this
...
IMAGE_TAG="${IMAGE_TAG:-rnnoise-ppa-builder}"
...
Build a source-only rnnoise package for ${SERIES} in an Ubuntu Podman
...
[ "${BRANCH}" = "rnnoise_ubuntu" ] \
    || warn "on branch ${BRANCH}, expected rnnoise_ubuntu"
...
[ "${SOURCE}" = "rnnoise" ] || die "unexpected source package: ${SOURCE}"
[ "${DISTRIBUTION}" = "${SERIES}" ] \
...
if podman image exists "${IMAGE_TAG}"; then
    IMAGE_CONTAINERFILE_SHA256="$(
        podman image inspect \
            --format '{{ index .Labels "org.rnnoise.containerfile-sha256" }}' \
...
if [ "${IMAGE_CONTAINERFILE_SHA256}" != "${CONTAINERFILE_SHA256}" ]; then
    ...
    podman build \
        --label "org.rnnoise.containerfile-sha256=${CONTAINERFILE_SHA256}" \
        --tag "${IMAGE_TAG}" \
        --file "${CONTAINERFILE}" \
        "${PACKAGE_DIR}"
fi
...
if [ "${UNSIGNED}" -eq 0 ]; then
    ...
    TEMP_GPG="$(mktemp -d "${TMPDIR:-/tmp}/rnnoise-gnupg.XXXXXX")"
