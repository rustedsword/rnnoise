...
WORK="$(mktemp -d /tmp/rnnoise-publish.XXXXXX)"
cleanup() {
...
if [ "${TEST_BUILD}" = "1" ]; then
    ...
    find "${BINARY_BUILD_DIR}/debian/librnnoise-dev" -type f \
        ! -name '*.h' | sed "s|${BINARY_BUILD_DIR}/debian/librnnoise-dev||"
    printf 'headers: %s\n' \
        "$(find "${BINARY_BUILD_DIR}/debian/librnnoise-dev" -name '*.h' 2>/dev/null | wc -l)"
fi
